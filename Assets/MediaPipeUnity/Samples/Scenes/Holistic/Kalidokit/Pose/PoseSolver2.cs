...
namespace Kalidokit
{
    public class PoseSolver2
    {
        public static PoseStruct Solve(Vector4[] poseLandmarks, Vector4[] poseLandmarks2D, bool EnableLegs = true)
        {
            ...
            bool rightHandOffscreen = poseLandmarks[15].y > -0.1 || poseLandmarks[15].w < 0.23 || 0.995 < poseLandmarks2D[15].y;
            bool leftHandOffscreen = poseLandmarks[16].y > -0.1 || poseLandmarks[16].w < 0.23 || 0.995 < poseLandmarks2D[16].y;
            bool rightFootOffscreen = poseLandmarks[23].y > -0.1 || poseLandmarks[23].w < 0.63 || hips.Hips.position.z > -0.4;
            bool leftFootOffscreen = poseLandmarks[24].y > 0.1 || poseLandmarks[24].w < 0.63 || hips.Hips.position.z > -0.4;
            
            arms.left.UpperArm = arms.left.UpperArm * (leftHandOffscreen ? 0 : 1);
            arms.left.UpperArm.z = leftHandOffscreen ? 1.5f : arms.left.UpperArm.z; //.55 is Hands down Default position
            arms.right.UpperArm = arms.right.UpperArm * (rightHandOffscreen ? 0 : 1);
            arms.right.UpperArm.z = rightHandOffscreen ? -1.5f : arms.right.UpperArm.z;
            arms.left.LowerArm = arms.left.LowerArm * (leftHandOffscreen ? 0 : 1);
            arms.right.LowerArm = arms.right.LowerArm * (rightHandOffscreen ? 0 : 1);
            arms.left.Hand = arms.left.Hand * (leftHandOffscreen ? 0 : 1);
            arms.right.Hand = arms.right.Hand * (rightHandOffscreen ? 0 : 1);
            ...
            if(EnableLegs)
            {
                ...
                result.LeftUpperLeg = legResult.Left.UpperLeg * (leftFootOffscreen ? 0 :1);
                result.LeftLowerLeg = legResult.Left.LowerLeg* (leftFootOffscreen ? 0 :1);
                result.RightUpperLeg = legResult.Right.UpperLeg* (rightFootOffscreen ? 0 :1);
                result.RightLowerLeg = legResult.Right.LowerLeg* (rightFootOffscreen ? 0 :1);
            }

            return result;

        }
        private static ArmResult CalcArms(Vector4[] poseLandmarks)
        {
        ...
        public static HipsResult CalcHips(Vector4[] poseLandmark, Vector4[] poseLandmark2D)
        {
        ...
        private static LegResult CalcLeg(Vector4[] poseLandmark)
        {
